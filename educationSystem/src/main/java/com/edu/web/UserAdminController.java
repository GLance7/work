...
import com.edu.model.JqgridBean;
...
public class UserAdminController {
        ...
        try {
            if(tuser.getId()!=null&&!tuser.getId().equals(0)){
                ...
                if(user==null){
                    ...
                    /*如果删除老师，则此老师教授的课程应全部删除（操作tcourse表）*/
                    Example tcourseExample = new Example(Tcourse.class);
                    tcourseExample.or().andEqualTo("teacherid",tuser.getId());
                    System.out.println("删除老师  "+tuser.getId());
                    tcourseService.deleteByExample(tcourseExample);


                    /*若删除的是学生，此学生的选课记录以及成绩应该完全删除（操作tselectcourse表）*/
                    Example tselectExample = new Example(Tselectcourse.class);
                    tselectExample.or().andEqualTo("studentid",tuser.getId());
                    tselectcourseService.deleteByExample(tselectExample);
    ...
    public String toselectUserById() {
        return "power/selectUserById111";
    }*/
    ...
    public Map<String, Object> selectUserById(Tuser tuser) {
        LinkedHashMap<String, Object> resultmap = new LinkedHashMap<String, Object>();
        System.out.println("找用户  "+tuser.getId());
        try {
            if(tuser.getId()!=null&&!tuser.getId().equals(0)){
...
//		logService.save(new Log(Log.LOGOUT_ACTION,"用户注销"));
    ...
    /*根据具体id来查找信息*/
    @ResponseBody
    @GetMapping("/getSpecificInfor")
    public List<Tuser> getSpecificInfor(Integer id) throws Exception{
        id=3;
        List<Tuser> userlistById = userService.selectSpecificInfor(id);
        System.out.println("查找信息 "+userlistById.toString());
        return userlistById;
    }

    /*根据remarks来查找学生和老师*/
    @ResponseBody
    @GetMapping("/getStudentOrTeacher")
    public List<Tuser> getStudentOrTeacher(Integer remarks) throws Exception{
        remarks=3;
        List<Tuser> userlistByremarks = userService.selectByRemarks(remarks);
        System.out.println("查找remarks "+userlistByremarks.toString());
        return userlistByremarks;
    }
